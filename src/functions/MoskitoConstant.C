...
#include "MoskitoConstant.h"
#include "MathUtils.h"

registerMooseObject("MooseApp", MoskitoConstant);

InputParameters
MoskitoConstant::validParams()
{
  InputParameters params = MoskitoTabularBase::validParams();
...
MoskitoConstant::MoskitoConstant(const InputParameters & parameters)
  : MoskitoTabularBase(parameters),
    _direction(getParam<MooseEnum>("direction").getEnum<Direction>())
{
}

Real
MoskitoConstant::value(Real t, const Point & p) const
{
...
ADReal
MoskitoConstant::value(const ADReal & t, const ADPoint & p) const
{
...
Real
MoskitoConstant::timeDerivative(Real /*t*/, const Point & /*p*/) const
{
  return 0;
}

Real
MoskitoConstant::integral() const
{
  const unsigned len = functionSize();
  Real sum = 0;
  unsigned offset = 0;
